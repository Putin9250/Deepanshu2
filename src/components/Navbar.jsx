...
import Logo from "../assets/react.svg"; // Update to your logo path
...
const Navbar = () => {
  ...
  // Scroll tracking for active section — loop from bottom to top
  useEffect(() => {
    const handleScroll = () => {
      const sections = Array.from(document.querySelectorAll("section"));
      const navHeight = navRef.current?.offsetHeight || 0;
      const scrollY = window.scrollY + navHeight + 5; // small offset for better accuracy

      // Find the last section where scrollY >= section top
  ...
  const scrollToSection = (e, href) => {
    ...
    if (section) {
      // Scroll smoothly accounting for navbar height
      const navHeight = navRef.current?.offsetHeight || 0;
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex justify-between items-center">
          {/* Logo */}
          <a href="#home" onClick={(e) => scrollToSection(e, "#home")} className="flex items-center gap-2 cursor-pointer">
